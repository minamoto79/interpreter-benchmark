...
public class HexanaVMConfig extends HotSpotVMConfigAccess {

    public HexanaVMConfig(HotSpotVMConfigStore config, Architecture arch) {
        super(config);
    ...
    // JDK17+ nmethod entry barrier: required for default JVMCI installs. The guard load must be an
    // ldr (literal) at the ENTRY_BARRIER_PATCH offset with a section_word reloc to a data-section int.
    ...
    public final int threadDisarmedGuardValueOffset =
            getFieldValue("CompilerToVM::Data::thread_disarmed_guard_value_offset", Integer.class, "int");
    public final long nmethodEntryBarrier =
            getFieldValue("CompilerToVM::Data::nmethod_entry_barrier", Long.class, "address");
