...
public abstract class TestAssembler {
    ...
    protected final HexanaVMConfig config;
    ...
    protected TestAssembler(CodeCacheProvider codeCache, HexanaVMConfig config, int initialFrameSize, int stackAlignment, PlatformKind narrowOopKind, Register... registers) {
    ...
    /**
     * Broker-path variant: use the in-flight compile request's id and entryBCI rather than
     * allocating a fresh id, so HotSpot associates the installed nmethod with the pending task.
     */
    public HotSpotCompiledCode finish(HotSpotResolvedJavaMethod method, int id, int entryBCI) {
        byte[] finishedCode = code.finish();
        Site[] finishedSites = sites.toArray(new Site[0]);
        byte[] finishedData = data.finish();
        DataPatch[] finishedDataPatches = dataPatches.toArray(new DataPatch[0]);
        int dataSectionAlignment = 8; // CodeBuffer::SECT_CONSTS code section alignment
        return new HotSpotCompiledNmethod(method.getName(), finishedCode, finishedCode.length, finishedSites, new Assumption[0], new ResolvedJavaMethod[]{method}, new Comment[0], finishedData, dataSectionAlignment,
                        finishedDataPatches, false, frameSize, deoptRescue, method, entryBCI, id, 0L, false);
    }
    ...
    protected static class Buffer {
        ...
        /** Backpatch a previously emitted 32-bit word (e.g. a branch offset) at absolute {@code pos}. */
        public void putIntAt(int pos, int value) {
